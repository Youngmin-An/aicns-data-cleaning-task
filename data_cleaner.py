...
if __name__ == "__main__":
    ...
    if app_conf["APP_STAGE_ENUM"].value >= Stage.DUP.value:
        ...
        save_to_data_warehouse(dedup_df, Stage.DUP, app_conf, time_col_name, data_col_name)
        if app_conf["APP_STAGE_ENUM"].value == Stage.DUP.value:
            save_to_data_warehouse(dedup_df, Stage.CLEANED, app_conf, time_col_name, data_col_name)
    print("dedup end")
    ...
    if app_conf["APP_STAGE_ENUM"].value >= Stage.OUTLIER.value:
        ...
        save_to_data_warehouse(deout_df, Stage.OUTLIER, app_conf, time_col_name, data_col_name)
        if app_conf["APP_STAGE_ENUM"].value == Stage.OUTLIER.value:
            save_to_data_warehouse(deout_df, Stage.CLEANED, app_conf, time_col_name, data_col_name)
    print("deoutlier end")
    ...
    if app_conf["APP_STAGE_ENUM"].value >= Stage.MISSING.value:
        ...
        save_to_data_warehouse(demiss_df, Stage.MISSING, app_conf, time_col_name, data_col_name)
        if app_conf["APP_STAGE_ENUM"].value == Stage.MISSING.value:
            save_to_data_warehouse(demiss_df, Stage.CLEANED, app_conf, time_col_name, data_col_name)
    print("demissing end")
